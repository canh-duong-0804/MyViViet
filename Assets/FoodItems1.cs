...
public class FoodItems1 : MonoBehaviour
{
    ...
    public static bool isOn1 = false;
    ...
    private void OnMouseDown()
    {
        ...
        {
            Debug.Log(gameControllerHue.instance.DishesListHue[foodIDInt].isUnLock + "Parsed successfully: " + foodIDInt);
        }
        if (!isOn1 && gameControllerHue.instance.DishesListHue[foodIDInt].isUnLock)
        {
            ...
            Sprite food = Resources.Load<Sprite>("FoodsDish/hue/" + foodID);
            textFood0.text = Resources.Load<TextAsset>("text/Newfolder/" + foodID).text;

            var temp = Resources.Load<TextAsset>("text/Newfolder/" + foodID + "-1");
            mota.text = temp.text;

            nguyenlieu.text = Resources.Load<TextAsset>("text/Newfolder/" + foodID + "-2").text;
            chebien.text = Resources.Load<TextAsset>("text/Newfolder/" + foodID + "-3").text;
            mota.enabled = false;
